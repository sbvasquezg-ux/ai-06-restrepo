...
\alert{Build parcial compilado; check --fast: exit 0. Cierre semántico pendiente.} La traducción de Prop. 2 confunde multiplicación y denominador; se conserva y señala, sin arreglar la copia a mano.
\sourcefoot{\NBER, Prop. 3 p. 13; copia literal del run GPT-5.6 Sol/xhigh, PaperInterface.lean y MainTheorems.lean.}
\end{frame}

% 16 -- 1:45
\begin{frame}{El veredicto debe sostenerse a mano}
\begin{columns}[T,onlytextwidth]
\column{.56\textwidth}
\small
\textbf{Lo que no aceptamos de una respuesta automática}: inferir salario decreciente a partir de participación decreciente.
\[
\partial_I W>0\iff P_I>D_I.
\]
\begin{keybox}\textbf{Veredicto}: el titular es falso como necesidad y correcto como posibilidad condicionada.\end{keybox}
\medskip
Antes de presentar: copiar la derivación, revisar la desigualdad, añadir la foto y recompilar. La evidencia manual todavía falta.
\column{.40\textwidth}
\IfFileExists{hand/manual-verification.png}{%
\includegraphics[width=\linewidth,height=.59\textheight,keepaspectratio]{hand/manual-verification.png}%
}{%
\begin{beamercolorbox}[sep=1em]{block body}
\textbf{Foto pendiente de la estudiante}\\[1em]
\small \texttt{hand/manual-verification.png}\\[1em]
La guía termina en la condición salarial. No se ha generado ni simulado una fotografía.
\end{beamercolorbox}}
\end{columns}
\sourcefoot{Derivación propia de \NBER, Prop. 3 p. 13 y ecs. (B9)--(B10), p. B-13; hand/README.md.}
\end{frame}
\end{document}
